...
function readInstanceId(): string {
  // Taken from validated configuration, not `process.env`, so an unvalidated
  // value can never reach a response body.
  return config.AETHER_INSTANCE_ID ?? 'unprovisioned';
}
...
async function readProcess(
  ...
  return {
    ...
    command,
    signalable: config.AETHER_PROCESS_SIGNAL_ENABLED && !protectedPids.has(pid),
  };
}
...
export async function listProcesses(options: ListProcessOptions): Promise<ProcessListResponse> {
  const emptyResponse: ProcessListResponse = {
    ...
    truncated: false,
    running: 0,
    signalEnabled: config.AETHER_PROCESS_SIGNAL_ENABLED,
  };
  ...
  const results = await Promise.all(
    numericPids.map((pid) => readProcess(pid, clockTicks, bootTimeMs, protectedPids))
  );
 ...
 */
export async function signalProcess(pid: number, signal: AllowedProcessSignal): Promise<void> {
  if (!config.AETHER_PROCESS_SIGNAL_ENABLED) {
    throw new ForbiddenError(
      'Process signalling is disabled on this instance. Set AETHER_PROCESS_SIGNAL_ENABLED=true to allow it.',
