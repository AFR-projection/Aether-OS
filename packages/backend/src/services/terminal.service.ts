 ...
 * working native module still serves the rest of the API, and so the failure is
 * reported as a clear 503 on the terminal routes rather than a crash at boot.
 */
...
export function isTerminalAvailable(): boolean {
  return ptyLoadError === null && config.TERMINAL_ENABLED;
}
...
function pruneScrollback(runtime: TerminalRuntime): void {
...
function resetIdleTimer(runtime: TerminalRuntime): void {
...
export async function createSession(options: CreateSessionOptions): Promise<TerminalSession> {
  ...
  if (sessions.size >= LIMITS.MAX_TERMINAL_SESSIONS_GLOBAL) {
    throw new ConflictError('The server has reached its terminal session limit');
  }

  if (countSessionsForUser(options.ownerUserId) >= config.TERMINAL_MAX_SESSIONS) {
    throw new ConflictError('You have reached your terminal session limit', {
      limit: config.TERMINAL_MAX_SESSIONS,
    });
  }
...
export function terminalStats(): { active: number; available: boolean; maxPerUser: number } {
  return {
    active: sessions.size,
    available: isTerminalAvailable(),
    maxPerUser: config.TERMINAL_MAX_SESSIONS,
  };
}

/** Resolves a workspace-relative path for display in the session listing. */
export async function relativeCwd(absolute: string): Promise<string> {
  const root = await getWorkspaceRoot();
