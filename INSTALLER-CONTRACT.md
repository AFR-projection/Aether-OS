...
This document defines the **explicit contract** between the Aether installer and the target system.
It specifies what the installer is allowed to do, what it requires, and what guarantees it provides.
...
> The installer MUST detect the operating system and MUST block installation on unsupported
> platforms with a clear error message and recommendations.
...
> The installer MUST detect the CPU architecture and MUST block installation on unsupported
> architectures.
...
> The installer MUST verify systemd is available and MUST block installation if systemd is not the
> init system.
...
> These resource requirements are **PROVISIONAL** and based on estimates. Actual requirements will
> be validated through testing. The installer may consume additional resources during installation.
...
> The installer MUST verify internet connectivity and DNS resolution. The installer MAY proceed
> without a public IP or domain, but will warn about limited accessibility.
...
> The installer MUST verify sudo availability and MUST block if sudo is not available or not
...
> The installer MUST use least privilege and MUST only escalate to root when necessary. All sudo
...
> The installer MUST NOT install packages beyond this list without explicit user consent. The
> installer MUST detect existing packages and skip installation.
...
> The installer MUST create this directory structure. The installer MUST set appropriate
> permissions:
>
...
> The installer MUST configure network isolation. Internal services MUST NOT be directly accessible
> from the internet.
...
> The installer MUST verify internal services are not exposed. The installer SHOULD include a
...
> The installer MUST:
>
> - Generate all secrets using cryptographically secure random number generator (openssl rand or
>   /dev/urandom)
...
> The installer MUST verify secret file permissions after creation and MUST fail if permissions
...
> The installer MUST generate a complete `.env` file. The installer MUST set `.env` permissions
> to 600.
...
> The installer MUST backup configuration before destructive changes. Backups MUST be timestamped
> and preserved.
...
> The installer MUST be safe to run multiple times. The installer MUST detect existing installation
> and offer appropriate options:
>
...
> The installer MUST save state after each stage. The installer MUST detect incomplete installation
> and offer resume.
...
> The installer MUST check for lock file and MUST refuse to run if another installer is active. The
> installer MUST remove lock file on exit (normal or error).
...
> The installer MUST distinguish between application files and user data. The installer MUST
> preserve user data on uninstall unless `--purge` is explicitly specified.
...
> The installer MUST provide safe uninstall that preserves data by default. The installer MUST
> require explicit `--purge` flag to delete user data. The installer MUST ask for confirmation
...
> The installer MUST respect existing Docker installation and MUST NOT reinstall or downgrade
> Docker.
...
> The installer MUST NOT stop or interfere with existing web servers. The installer MUST detect
> conflicts and require user decision.
...
> The installer MUST detect existing Aether installation and MUST offer appropriate actions. The
> installer MUST NOT corrupt existing installation.
...
> Updates MUST be non-destructive. Updates MUST backup configuration before modifying. Updates MUST
...
> Rollback MUST restore previous working state. Rollback MUST NOT delete data. If database migration
> is incompatible, rollback MUST warn and provide manual steps.
...
> The installer MUST log all operations. Logs MUST NOT contain secrets. Logs MUST be preserved after
...
> The installer MUST maintain audit trail. Audit trail MUST be machine-readable JSON. Audit trail
...
> The installer MUST enforce download security. The installer MUST NOT proceed if verification
> fails.
...
> The installer MUST reject invalid input. The installer MUST NOT trust user input without
> validation.
...
> The installer MUST distinguish between recoverable and non-recoverable failures. The installer
> MUST retry recoverable failures up to 3 times. The installer MUST abort non-recoverable failures
> immediately.
...
> The installer MUST generate unique installation ID. The ID MUST be displayed to user for support
> purposes.
