...
- React escapes by default; `dangerouslySetInnerHTML` does not appear in this codebase, and new code
  should not introduce it.
- The backend sets a strict CSP via `@fastify/helmet` — `script-src 'self'`, no `unsafe-inline` for
  scripts. See [SECURITY-MODEL.md](../security/SECURITY-MODEL.md).
- Terminal output renders through xterm.js into a canvas, not the DOM.
