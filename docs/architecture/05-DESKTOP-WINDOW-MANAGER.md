...
The Desktop Shell is the core UI that users interact with - taskbar, launcher, system tray,
notifications.
...
class WindowManager {
  ...
  minimizeWindow(windowId: string): void {
    ...
    const nextWindowId = this.focusStack.find(
      (id) => id !== windowId && this.windows.get(id)?.visible
    );
